...
<body class="bg-slate-900 text-white font-sans min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex justify-between items-center">
        ...
        <h1 class="text-2xl font-bold">{{ __('La teva cistella') }} 🛒</h1>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @if(!$cistella || $cistella->llibres->isEmpty())
            ...
            <div class="text-center py-20 bg-slate-800 rounded-3xl border border-slate-700">
                ...
                <h2 class="text-2xl font-bold mb-2">{{ __('La teva cistella està buida') }}</h2>
                <p class="text-slate-400 mb-6">{{ __('Sembla que encara no has afegit cap llibre.') }}</p>
                <a href="{{ route('home') }}" class="bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-8 rounded-full transition">{{ __('Explorar el catàleg') }}</a>
            </div>
        @else
            {{-- Taula de Productes --}}
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-4">
                    ...
                    <div class="relative flex items-center bg-slate-800 p-4 rounded-xl border border-slate-700 group hover:border-blue-500/50 transition">
                        ...
                        <form action="{{ route('cistella.eliminar', $llibre->id_llibre) }}" method="POST" class="absolute top-2 right-2 z-20">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-slate-500 hover:text-red-500 transition p-1 rounded-full hover:bg-slate-700" title="{{ __('Eliminar') }}">
                        ...
                        <a href="{{ route('llibres.show', $llibre->id_llibre) }}" class="flex items-center flex-grow mr-4 z-10">
                            {{-- Imatge (CORREGIT: img/ en lloc de storage/) --}}
                            ...
                            <div class="ml-4 flex-grow pr-8">
                                <h3 class="font-bold text-lg text-white hover:text-blue-400 transition">{{ __($llibre->titol) }}</h3>
                                <p class="text-slate-400 text-sm">{{ $llibre->autor ? $llibre->autor->nom : __('Autor Desconegut') }}</p>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-slate-800 p-6 rounded-2xl border border-slate-700 sticky top-10">
                        <h3 class="text-xl font-bold mb-6 border-b border-slate-700 pb-4">{{ __('Resum de la comanda') }}</h3>
                        ...
                        <div class="flex justify-between mb-6 text-slate-400">
                            <span>{{ __('Impostos (inclosos)') }}</span>
                            <span>0.00 €</span>
                        </div>
