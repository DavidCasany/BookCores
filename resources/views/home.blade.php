...
<body class="antialiased bg-slate-50 text-slate-800">
    ...
    <div class="relative bg-white overflow-hidden">
        ...
        <div class="relative w-full aspect-[3/4] md:aspect-[16/9] lg:aspect-auto lg:h-auto lg:absolute lg:inset-y-0 lg:right-0 lg:w-1/2 bg-slate-900 overflow-hidden z-0 group"
             x-data="{
                activeSlide: 0,
                totalSlides: {{ $llibresRecents->count() }},
                autoplay: null,
                startAutoplay() {
            ...
            <div class="flex h-full transition-transform duration-1000 ease-in-out will-change-transform"
                 :style="'transform: translateX(-' + (activeSlide * 100) + '%)'">

                @forelse($llibresRecents as $llibreRecent)
                    <div class="relative w-full h-full flex-shrink-0">
                        @if($llibreRecent->img_portada)
                            <img src="{{ asset('img/' . $llibreRecent->img_portada) }}"
                                 alt="{{ $llibreRecent->titol }}"
                                 class="object-cover w-full h-full"
                                 onerror="this.src='https://placehold.co/800x1200/1e293b/475569?text=BookCores'">
                        ...
                        <div class="absolute bottom-8 left-8 right-8 z-20 pointer-events-none">
                            <h3 class="text-white text-xl md:text-2xl font-bold truncate drop-shadow-lg">{{ $llibreRecent->titol }}</h3>
                        </div>
                    </div>
                @empty
                    <div class="relative w-full h-full flex-shrink-0 bg-slate-800 flex items-center justify-center text-slate-500">
                        <p>{{ __('Encara no hi ha llibres recents.') }}</p>
                    </div>
                @endforelse
            </div>
        </div>
    ...
    <div id="novetats" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 bg-slate-50 relative z-10">
                ...
                <div class="group relative bg-white border border-slate-200 rounded-xl p-4 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
                    <div class="aspect-w-2 aspect-h-3 w-full overflow-hidden rounded-lg bg-slate-200 mb-4 relative shadow-inner">
                        ...
                        <span class="absolute top-2 right-2 bg-white/90 backdrop-blur text-slate-900 text-xs font-bold px-2 py-1 rounded-full shadow-sm flex items-center gap-1">
                            <svg class="w-3 h-3 text-yellow-500" fill="currentColor" viewBox="0 0 0 20 20">
                    ...
                    <div class="flex flex-col flex-grow">
                        ...
                        <div class="mt-auto pt-4 flex items-center justify-between">
                            ...
                            <button class="relative z-10 p-2 rounded-full bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition-colors duration-200" title="{{ __('Afegir al carretó') }}">
